...
use Jirro\Component\Http\Route\RouteAwareInterface;
use Jirro\Component\Http\Route\RouteAwareTrait;
use Jirro\Component\ORM\Tools\Pagination\Paginator;

class ResourcesController extends StandardController implements RouteAwareInterface
{
    use RouteAwareTrait;
    ...
    public function indexAction(Request $request)
    {
        ...
        $queryBuilder = $this
            ...
            ->createQueryBuilder()
            ->select('resources')
            ->from('Jirro\Bundle\AccountBundle\Domain\Resource', 'resources')
        ...
        if ($filters) {
            if ($filters->get('name') !== null && $filters->get('name') !== '') {
                $queryBuilder
                    ->andWhere('UPPER(resources.name) = :name')
        ...
        return new Response($this->getView()->render('Admin::resources/index', [
            'paginator' => $paginator,
            'filters'   => $filters,
        ]));
    }
